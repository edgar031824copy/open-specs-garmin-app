...
Deployment target: Render free tier (backend) + Netlify free tier (frontend), zero cost, minimal config.

## Goals / Non-Goals

**Goals:**
- Backend accessible at a stable Render URL
- Frontend accessible at a stable Netlify URL
- Frontend talks to the deployed backend via `VITE_API_URL`
- CORS restricted to the Netlify origin in production
...
Netlify's free tier hosts static files with global CDN. Build command: `npm run build` inside `apps/garmin-training/client/`. Publish dir: `apps/garmin-training/client/dist`. Add `netlify.toml` at `apps/garmin-training/client/` for SPA routing (all routes redirect to `index.html`).

`VITE_API_URL` is set as a Netlify env var pointing to the Render service URL — baked into the client bundle at build time by Vite.

### CORS restriction in production
Current `cors()` call is open to all origins. In production the backend should restrict to the Netlify domain. Use `ALLOWED_ORIGIN` env var on Render — if set, pass it as `origin` to the cors middleware; otherwise fall back to open (preserves local dev behavior without code changes).
...
- `ALLOWED_ORIGIN` — Netlify frontend URL for CORS
...
**`VITE_API_URL` baked at build time** → Changing the Render URL requires a Netlify rebuild. Mitigation: Render URLs are stable once created; not a practical issue for this PoC.
