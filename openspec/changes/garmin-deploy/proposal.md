...
- Deploy React/Vite frontend to Vercel free tier (static site)
- Expose backend environment variables (Supabase, Garmin credentials, Anthropic API key) via Render's environment settings
- Configure CORS on the backend to accept requests from the Vercel domain
...
### New Capabilities
- `garmin-deployment`: Production deployment configuration for the Garmin Training app — Render backend + Vercel frontend, environment wiring, CORS policy, and deploy scripts
...
- `apps/garmin-training/client/` — add `.env.production` with `VITE_API_URL`, confirm Vercel build config
...
- Custom domain or TLS beyond Render/Vercel defaults
