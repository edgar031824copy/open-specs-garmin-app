...
### Requirement: Frontend deployed on Vercel
The system SHALL serve the React/Vite client as a Vercel static site with a stable public URL.
...
- **THEN** Vercel serves `index.html` instead of returning 404
...
### Requirement: CORS restricted to Vercel origin
The backend SHALL restrict CORS to the Vercel frontend domain in production.

#### Scenario: Requests from Vercel domain succeed
- **WHEN** the frontend at the Vercel URL sends an API request
